...
echo "ITEM: ATOMS id type xu yu zu c_q[1] c_q[2] c_q[3] c_q[4] c_shape[1] c_shape[2] c_shape[3] vx vy vz angmomx angmomy angmomz mol" >> header
 
# delete the header + sort the atoms.
tail -n+10 npb_substrate.orig | sort -n > dump.tail

# create extra columns with the mol index
for i in {1..200}; do for j in {1..5}; do echo " 0. 0. 0. 0. 0. 0. $i"; done; done > mol
